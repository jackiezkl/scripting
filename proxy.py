...
def server_loop(local_host,local_port,remote_host,remote_port,receive_first):
	...
	try:
		...
		print "[!] Check for other listening sockers or correct permissions."
		sys.exit(0)
	...
	while True:
		...
		print "[==>] Received incoming connection rom %s:%d" (addr[0],addr[1])
		#start a thread to talk to the remote host
		proxy_thread = threadingThread(target = proxy_handler, args = (client_socket, remote_host, remote_port, receive_first))
		proxy_thread.start()
def main():
	if len(sys.argv[1:]) != 5:
	...
	#the following tells the proxy to connect and receive data before
